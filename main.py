...
async def collaborative_answer_mode_endpoint(
    ...
    if not active_session.id:
        ...
        db.commit()
        db.refresh(active_session)
        logger.info(f"新規チャットセッションをDBに保存しID確定: ID={active_session.id}")
    ...
    if requested_output_format_str and response_shell.step7_final_answer_v2_openai and response_shell.step7_final_answer_v2_openai.response:
        ...
        if requested_output_format_str == "markdown":
            actual_extension = "md"
        elif requested_output_format_str == "python":
            actual_extension = "py"
        elif requested_output_format_str == "javascript":
            actual_extension = "js"
        ...
        if actual_extension in supported_text_extensions:
            try:
                ...
                # 生成成功メッセージをAIの応答に追記（任意）
                # success_output_message = f"\n\n（システムより追記: ご要望の「{requested_output_format_str}」形式でファイル「{output_filename_with_ext}」を生成しました。チャット下部からダウンロード可能です。）"
                # if response_shell.step7_final_answer_v2_openai.response:
                #    response_shell.step7_final_answer_v2_openai.response += success_output_message
...
"""
        ...
        # logger.info(f"OpenAI 第2最終回答 (冒頭): {step7_res.response[:100].strip()}...") # この行は有効なはず
