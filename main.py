...
FRIENDLY_TONE_SYSTEM_PROMPT = (
    "あなたは、提供された情報やレビューを基に、ユーザーの最初の質問に対して最終的な回答を執筆する、非常にフレンドリーで親しみやすいAIライターだ！"
    "最終的な回答は、まるで親しい友人や家族に話しかけるように、とても軽くてラフな口調でお願い。"
    "例えば、語尾は「～なんだ！」「～だ！」「～って感じかな？」「～かもね！」みたいな感じで、読者がリラックスして楽しく読める文章を目指してほしいな。"
    "そしてここが重要なんだけど、回答は**とにかく詳しく、情報量たっぷりで、具体例もたくさん交えて、ボリューム満点**にしてほしいんだ。"
    "ユーザーが「なるほど、そういうことか！」「へぇ～、面白い！」って思わず言っちゃうくらい、深掘りして、背景や細かい点まで分かりやすく説明してね。"
    "専門用語を使うなら「分かりやすくいうと～ってことなんだ」みたいに、友達に教える感じで噛み砕いて説明してほしい。"
    "読者が満足してくれるような、読み応えのあるしっかりとした内容を期待してるよ！"
    "これまでのAIたちの努力の結晶を、最高の形で、そして最高の親しみやすさでユーザーに届けて！"
)
...
上記の全情報を踏まえ、特にClaudeによる最終レビュー（項目6）の指摘を全て解消し、指定されたフレンドリーかつ情報満載の口調で、ユーザーの現在の質問に対する完璧な最終回答を作成してください。
"""
...
「第1最終回答」を元にしつつ、もっともっと分かりやすくて、面白くて、そして何より【超フレンドリーでラフな口調】で、かつ【情報量たっぷりでボリューム満点の詳しい説明】で、ユーザーさんが「なるほどー！めっちゃ分かりやすいし、詳しい！」って感動するような「完成版」の回答を作ってほしいんだ。
細かい点や背景、具体例もたくさん入れて、読み応えのある回答をよろしくね！頼んだよ！🎉
"""
        step7_res = await get_openai_response( # ★★★ get_openai_response を呼び出し ★★★
            prompt_text=step7_user_prompt_for_openai_final,
            system_role_description=FRIENDLY_TONE_SYSTEM_PROMPT, # FRIENDLY_TONE_SYSTEM_PROMPT はグローバルで定義
            model="gpt-4o"
...
"""
        ...
        if user_specified_tone:
            ...
            w5_system_instruction_parts.append(FRIENDLY_TONE_SYSTEM_PROMPT) # デフォルトのフレンドリー口調
