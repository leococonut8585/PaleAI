...
def format_code_mode_output(code: str, explanation: str, tests: str, language: str = "python") -> str:
...
async def run_code_mode_flow(
    ...
    logger.info(f"Code Mode: このターンでAIに渡す完全な履歴は {len(current_chat_history_for_this_turn)} 件, メモリ: {len(user_memories) if user_memories else 0}件")

    try:
        logger.info("コードモード ステップC0: ユーザープロンプトの精密化中...")
        ...
        c0_user_prompt = (f"現在のユーザーリクエストは「{original_prompt}」です。\n"
                          "このリクエストに基づいてコードを生成するために必要な情報を、以下の項目でできる限り詳細に、かつ構造化して整理してください。\n")
        c0_res = await get_gemini_response(
            request=request, prompt_text=c0_user_prompt, system_instruction=c0_system_instruction, model_name="gemini-2.5-pro-preview-05-06",
        ...
        if c0_res.error or not c0_res.response: raise ValueError(f"コードモード ステップC0 (プロンプト精密化) 失敗: {c0_res.error or '応答がありませんでした。'}")
        ...
        for line in refined_requirements.splitlines():
            if line.lower().startswith("言語:") or line.lower().startswith("language:"):
                ...
                if lang_candidate: detected_language = lang_candidate
        ...
        c1_user_prompt = (f"以下のプログラム要件（ステップC0で明確化されたもの）があります:\n--- 要件ここから ---\n{refined_requirements}\n--- 要件ここまで ---\n"
                          "この要件を満たすための詳細な技術仕様、主要なロジックの設計（関数名や役割など）、そして中心となる処理の疑似コードを作成してください。")
        c1_res = await get_claude_response(
            request=request, prompt_text=c1_user_prompt, system_instruction=c1_system_instruction, model="claude-opus-4-20250514",
            chat_history=list(current_chat_history_for_this_turn), initial_user_prompt=initial_user_prompt_for_session, user_memories=user_memories,
        )
        steps_executed.append(c1_res)
        if c1_res.error or not c1_res.response: raise ValueError(f"コードモード ステップC1 (詳細仕様策定) 失敗: {c1_res.error or '応答がありませんでした。'}")
        ...
        c2_user_prompt = (f"以下の詳細仕様と疑似コードに基づいて、{detected_language}でコードを生成してください。\n\n"
                          f"--- 詳細仕様と疑似コードここから ---\n{detailed_specs_and_pseudo}\n--- 詳細仕様と疑似コードここまで ---\n\n"
                          f"生成するコードは、上記の仕様とロジックを正確に実装し、適切な変数名、関数名、そして理解を助けるコメントを{detected_language}の慣習に従って含めてください。")
        c2_res = await get_openai_response(
            request=request, prompt_text=c2_user_prompt, system_role_description=c2_system_role_description, model="gpt-4o",
            chat_history=list(current_chat_history_for_this_turn), initial_user_prompt=initial_user_prompt_for_session, user_memories=user_memories,
        )
        steps_executed.append(c2_res)
        if c2_res.error or not c2_res.response: raise ValueError(f"コードモード ステップC2 (第1コード生成) 失敗: {c2_res.error or '応答がありませんでした。'}")
        ...
        c3_user_prompt = (f"以下の{detected_language}コードについて、詳細なレビューと具体的な改善提案をお願いします。\n\n"
                          f"--- コードここから ---\n{generated_code_v1}\n--- コードここまで ---\n")
        c3_res = await get_claude_response(
            request=request, prompt_text=c3_user_prompt, system_instruction=c3_system_instruction, model="claude-opus-4-20250514",
            chat_history=list(current_chat_history_for_this_turn), initial_user_prompt=initial_user_prompt_for_session, user_memories=user_memories,
        )
        steps_executed.append(c3_res)
        if c3_res.error or not c3_res.response: logger.info(f"コードモード ステップC3 (コードレビュー) でエラーまたは応答なし: {c3_res.error or '応答なし'}"); code_review_feedback = "AIによるコードレビューはありませんでした。"
        else: code_review_feedback = c3_res.response
        ...
        c4_user_prompt = (f"以下のオリジナルの{detected_language}コードと、それに対するレビューおよび改善提案があります。\n\n"
                          f"--- オリジナルコード (V1) ここから ---\n{generated_code_v1}\n--- オリジナルコード (V1) ここまで ---\n\n"
                          f"--- レビューと改善提案 ここから ---\n{code_review_feedback}\n--- レビューと改善提案 ここまで ---\n\n"
                          f"上記のレビューと改善提案をすべて慎重に検討し、指摘された点を修正・反映した改善版の{detected_language}コード（バージョン2）を生成してください。")
        c4_res = await get_openai_response(
            request=request, prompt_text=c4_user_prompt, system_role_description=c4_system_role_description, model="gpt-4o",
            chat_history=list(current_chat_history_for_this_turn), initial_user_prompt=initial_user_prompt_for_session, user_memories=user_memories,
        )
        steps_executed.append(c4_res)
        if c4_res.error or not c4_res.response: raise ValueError(f"コードモード ステップC4 (改善版コード生成) 失敗: {c4_res.error or '応答がありませんでした。'}")
        ...
        c5_user_prompt = (f"以下の{detected_language}コードについて、詳細なテストケースを提案してください。\n\n"
                          f"--- 対象コード (バージョン2) ---\n{generated_code_v2}\n--- 対象コードここまで ---\n\n"
                          "上記のシステム指示に従い、できるだけ多くの観点から、具体的で検証可能なテストケースを提案してください。")
        c5_res = await get_claude_response(
            request=request, prompt_text=c5_user_prompt, system_instruction=c5_system_instruction, model="claude-opus-4-20250514",
            chat_history=list(current_chat_history_for_this_turn), initial_user_prompt=initial_user_prompt_for_session, user_memories=user_memories,
        )
        steps_executed.append(c5_res)
        if c5_res.error or not c5_res.response: logger.info(f"コードモード ステップC5 (テストケース提案) でエラーまたは応答なし: {c5_res.error or '応答なし'}"); test_cases_suggestion = "AIによるテストケースの提案はありませんでした。"
        else: test_cases_suggestion = c5_res.response
        ...
        c6_user_prompt = (f"以下の{detected_language}コードについて、詳細な使い方と解説をお願いします。\n\n"
                          f"--- 対象コード (バージョン2) ---\n{generated_code_v2}\n--- 対象コードここまで ---\n\n"
                          "上記のシステム指示に従い、このコードを初めて見る人でも理解しやすく、すぐに使えるような親切な解説を作成してください。")
        c6_res = await get_openai_response(
            request=request, prompt_text=c6_user_prompt, system_role_description=c6_system_role_description, model="gpt-4o",
            chat_history=list(current_chat_history_for_this_turn), initial_user_prompt=initial_user_prompt_for_session, user_memories=user_memories,
        )
        steps_executed.append(c6_res)
        if c6_res.error or not c6_res.response: logger.info(f"コードモード ステップC6 (コード解説) でエラーまたは応答なし: {c6_res.error or '応答なし'}"); code_explanation = "AIによるコードの解説はありませんでした。"
        else: code_explanation = c6_res.response
        logger.info(f"ステップC6 - コード解説 (冒頭):\n{code_explanation[:300].strip()}...")

        final_code_mode_output = format_code_mode_output(generated_code_v2, code_explanation, test_cases_suggestion, language=detected_language)
        response_shell.step7_final_answer_v2_openai = schemas.IndividualAIResponse(source="Code Mode (Full Flow C0-C6)", response=final_code_mode_output)
        ...
        if not response_shell.step7_final_answer_v2_openai:
            response_shell.step7_final_answer_v2_openai = schemas.IndividualAIResponse(
                source="Code Mode Error Step", error=str(e), response=f"申し訳ありません、コード生成処理中にエラーが発生しました。\nエラー内容: {str(e)}",
            )
    return response_shell
